...
class MACNetwork(nn.Module):
    def __init__(self, n_vocab, dim, embed_hidden=512,
                max_step=12, self_attention=False, memory_gate=False,
                classes=1, dropout=0.15):
        super().__init__()
    ...
    def forward(self, graph, question, question_len, dropout=0.15):
        b_size = question.size(0)

        graph = graph.view(b_size, self.dim, -1)
        ...
        memory = self.mac(lstm_out, h, graph)
...
class MACUnit(nn.Module):
    ...
    def forward(self, context, question, knowledge):
        ...
        for i in range(self.max_step):
            ...
            # print(memories[0].shape, knowledge.shape, controls)
